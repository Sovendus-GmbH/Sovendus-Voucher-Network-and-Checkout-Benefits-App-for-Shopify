...
${errorInfo.componentStack}`);
  ...
  function getBannerData({
    ...
    const url = new URL(
      "https://sovendus-press.testing6.sovendus.com/banner/api/banner/json"
    );
    const searchParams = __spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues({
      trafficSourceNumber,
      trafficMediumNumber
    }, orderConfirmationNumber && { orderId: orderConfirmationNumber }), netOrderValue && { orderValue: String(netOrderValue) }), currency.isoCode && { orderCurrency: currency.isoCode }), voucherCode && { usedCouponCode: voucherCode }), addressData.firstName && { firstName: addressData.firstName }), addressData.lastName && { lastName: addressData.lastName }), addressData.zip && { zip: addressData.zip }), addressData.city && { city: addressData.city }), email && { email }), phone && { phone }), addressData.countryCode && { country: addressData.countryCode });
    Object.entries(searchParams).forEach((item) => {
      url.searchParams.set(...item);
    });
    console.log("requestBody", searchParams);
    fetch(url.toString()).then((response) => {
      return response.json();
    }).then((response) => {
      console.log("response", response);
      setBannerData({ nativeBanner: response });
    }).catch((error) => {
      console.log("Failed to get sovendus response data error: ", error);
    });
  }
